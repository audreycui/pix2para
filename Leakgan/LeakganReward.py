...
class Reward(object):
    ...
    def get_reward(self, input_x, conv_features):
        rewards = []
        for i in range(self.rollout_num):
            ...
            feed = {self.dis.D_input_x: input_x, 
                    self.dis.conv_features:np.zeros((self.model.batch_size, self.model.image_feat_dim), dtype=np.float32)}
            ypred_for_auc = self.sess.run(self.dis.ypred_for_auc, feed)
            ypred = np.array([item[1] for item in ypred_for_auc])
            if i == 0:
                rewards.append(ypred)
