...
'''
Added: 
#conv_features placeholder (for image features)
#xW+b operation for image features to create image embedding
#lstm encoder for image embedding
#final state of encoder becomes initial states of worker decoder lstm and manager decoder lstm
'''
class Generator(object):
    def __init__(self,config, D_model):
        ...
        with tf.variable_scope('place_holder'):
            self.conv_features = tf.placeholder(dtype = tf.float32, shape = [self.batch_size, self.image_feat_dim], name="conv_features")
            #self.conv_features = tf.zeros([self.batch_size, self.image_feat_dim])
        ...
        with tf.variable_scope('Worker'):
            ...
            self.encoder = tf.nn.rnn_cell.LSTMCell(hidden_dim, state_is_tuple=True)
            #self.worker_params.append(self.encoder)

            random_uniform_init = tf.random_uniform_initializer(minval=-0.1, maxval=0.1)
            image_feat_W = tf.get_variable("image_feat_W", [self.image_feat_dim, self.emb_dim], tf.float32, random_uniform_init)
            image_feat_b = tf.get_variable("image_feat_b", [self.emb_dim], tf.float32, random_uniform_init)

            #self.g_worker_decoder = tf.nn.rnn_cell.LSTMCell(hidden_dim)
        ...
        state = self.encoder.zero_state(self.batch_size, tf.float32)
        with tf.variable_scope("image_feat") as scope:
            #image_feat = self.batch_norm(self.conv_features[:,:], mode='test', name='')
            image_feat_emb = tf.matmul(self.conv_features, image_feat_W) + image_feat_b 

        with tf.variable_scope("lstm_encoder") as scope:
            _, state = self.encoder(image_feat_emb, state)
            lstm_c, lstm_h = state
            #self.encode_state = tf.stack([lstm_h, lstm_c])  #where h0_worker is the initial state for decoder
            #print("encoder state:"+str(tf.shape(state)))
            #print("h0 worker:"+str(tf.shape(self.h0_worker)))

        self.encode_state = tf.stack([lstm_h, lstm_c])
        #self.encode_state = self.h0_worker
        # processed for batch
        ...
        def _g_recurrence(i, x_t, h_tm1, h_tm1_manager, gen_o, gen_x, goal, last_goal, real_goal, step_size,
            ...
            h_t_manager = h_tm1_manager #self.g_manager_recurrent_unit(feature, h_tm1_manager)
        ...
        _, _, _, _, self.gen_o, self.gen_x, _, _, _, _, self.gen_real_goal_array, self.gen_o_worker_array = control_flow_ops.while_loop(
            ...
            loop_vars=(tf.constant(0, dtype=tf.int32),
                       tf.nn.embedding_lookup(self.g_embeddings, self.start_token), self.encode_state, self.h0_manager,
        ...
        def preTrain(i, x_t, g_predictions, h_tm1, input_x, h_tm1_manager, last_goal, real_goal, feature_array,
            ...
            with tf.variable_scope(self.scope):
                feature = self.FeatureExtractor_unit(cur_sen, self.drop_out, self.conv_features)
        ...
        self.pretrain_summary = tf.summary.merge([
            tf.summary.scalar("pretrain_worker_loss", self.pretrain_worker_loss),
            tf.summary.scalar("pretrain_loss", self.pretrain_loss)])

        self.g_summary = tf.summary.merge([
            tf.summary.scalar("g_loss", self.goal_loss),
            tf.summary.scalar("w_loss", self.worker_loss)])
    ...
    def pretrain_step(self, sess, x, conv_features, dropout_keep_prob):
        if conv_features is not None:
            outputs = sess.run([self.pretrain_worker_updates, self.pretrain_worker_loss, self.pretrain_manager_updates,
                            self.pretrain_goal_loss, self.pretrain_summary],
                           feed_dict={self.x: x, 
            ...
            outputs = sess.run([self.pretrain_worker_updates, self.pretrain_worker_loss, self.pretrain_manager_updates,
                           ...
                           feed_dict={self.x: x, 
                                      self.drop_out: dropout_keep_prob,
                                      self.conv_features: np.zeros((self.batch_size, self.image_feat_dim), dtype=np.float32)})
        return outputs

    def get_nll(self, sess, x, conv_features):
        dropout_keep_prob = .8
        outputs = sess.run([self.pretrain_worker_updates, self.pretrain_worker_loss, self.pretrain_manager_updates,
                            self.pretrain_goal_loss],
                           feed_dict={self.x: x, self.conv_features: conv_features, self.drop_out: dropout_keep_prob})
        return outputs[1]

    def generate(self, sess, conv_features, dropout_keep_prob, train=1):
        if conv_features is not None:
            outputs = sess.run(self.gen_x, feed_dict={self.drop_out: dropout_keep_prob, self.train: train, self.conv_features: conv_features})
        else:
            outputs = sess.run(self.gen_x, feed_dict={self.drop_out: dropout_keep_prob, self.train: train, self.conv_features: np.zeros((self.batch_size, self.image_feat_dim), dtype=np.float32)})
        return outputs
